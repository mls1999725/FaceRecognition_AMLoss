...
class miniloss(nn.Module):
    ...
    def __init__(self, in_features, out_features, s=30.0):
        ...
        self.out_fearures = out_features
        self.s = s
    ...
    def forward(self, input, label):
        ...
        alpha = m1/2 * torch.cos(2 * theta + math.pi) + m1/2
        beta = m2/2 * torch.cos(2 * theta) + m2/2
        phi = cosine - alpha
        # phi = cosine - torch.abs(cosine - alpha)
        phj = cosine + beta
...
class MultiMini(nn.Module):
    def __init__(self, in_features, out_features, num_sphere=4, s=30.0):
        super(MultiMini, self).__init__()
        self.in_features = in_features
        self.out_fearures = out_features
        self.num_sphere = num_sphere
        self.s = s
        self.weight_list = []
        self.each_in_features = in_features // num_sphere
        for i in range(num_sphere):
            weight = Parameter(torch.FloatTensor(out_features, self.each_in_features))
            self.weight_list.append(weight)
            nn.init.xavier_uniform_(self.weight_list[i])
        # nn.init.xavier_uniform_(self.weight)

    def forward(self, input, label):
        output_list = []
        for i in range(self.num_sphere):
            cosine = F.linear(F.normalize(input[:, i * self.each_in_features:(i + 1) * self.each_in_features]), F.normalize(self.weight_list[i].cuda()))
            m1 = 0.3
            m2 = 0.0
            theta = torch.acos(cosine)
